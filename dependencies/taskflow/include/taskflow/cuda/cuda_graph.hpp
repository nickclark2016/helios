...
namespace helios
{
    ...
    class cudaNode
    {
        ...
        using handle_t = nstd::variant<nstd::monostate, Noop,
                                       // Host,
                                       Memset, Copy, Kernel>;
    ...
    inline void cudaGraph::clear_native_graph()
    {
        if (_native_handle)
        {
            TF_CHECK_CUDA(cudaGraphDestroy(_native_handle), "failed to destroy a cudaGraph on clear");
    ...
    inline void cudaGraph::_make_native_graph()
    {
        ...
        TF_CHECK_CUDA(cudaGraphCreate(&_native_handle, 0), "failed to create a cudaGraph");
        ...
        for (auto& node : _nodes)
        {
            ...
            {
            case cudaNode::NOOP:
                nstd::get<cudaNode::Noop>(node->_handle).work(_native_handle, node->_native_handle);
                break;
            ...
            case cudaNode::MEMSET:
                nstd::get<cudaNode::Memset>(node->_handle).work(_native_handle, node->_native_handle);
                break;

            case cudaNode::COPY:
                nstd::get<cudaNode::Copy>(node->_handle).work(_native_handle, node->_native_handle);
                break;

            case cudaNode::KERNEL:
                nstd::get<cudaNode::Kernel>(node->_handle).work(_native_handle, node->_native_handle);
                break;
            }
        }
        ...
        for (auto& node : _nodes)
        {
            ...
            {
                TF_CHECK_CUDA(
                    ::cudaGraphAddDependencies(_native_handle, &(node->_native_handle), &(succ->_native_handle), 1),
