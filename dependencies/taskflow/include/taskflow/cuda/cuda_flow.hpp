...
namespace helios
{
    ...
    */
    ...
    public:
        ...
        */
        template <typename T>
        std::enable_if_t<is_pod_v<T> && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4), cudaTask> zero(
            T* dst, size_t count);
        ...
        */
        template <typename T>
        std::enable_if_t<is_pod_v<T> && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4), cudaTask> fill(
            T* dst, T value, size_t count);
    ...
    // Constructor
    template <typename P>
    cudaFlow::cudaFlow(cudaGraph& g, P&& p) : _graph{g}, _predicate{std::forward<P>(p)}
    {
    }
    ...
    {
        auto node =
            _graph.emplace_back(nstd::in_place_type_t<cudaNode::Noop>{}, [](cudaGraph_t& graph, cudaGraphNode_t& node) {
                TF_CHECK_CUDA(::cudaGraphAddEmptyNode(&node, graph, nullptr, 0),
    ...
    cudaTask cudaFlow::kernel(dim3 g, dim3 b, size_t s, F&& f, ArgsT&&... args)
    {
        ...
        auto node = _graph.emplace_back(nstd::in_place_type_t<cudaNode::Kernel>{},
                                        [g, b, s, f = (void*)f, args...](cudaGraph_t& graph, cudaGraphNode_t& node) {
                                            ...
                                            TF_CHECK_CUDA(::cudaGraphAddKernelNode(&node, graph, nullptr, 0, &p),
    ...
    // Function: kernel
    template <typename F, typename... ArgsT>
    cudaTask cudaFlow::kernel_on(int d, dim3 g, dim3 b, size_t s, F&& f, ArgsT&&... args)
    {
    ...
    // Function: zero
    template <typename T>
    std::enable_if_t<is_pod_v<T> && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4), cudaTask> cudaFlow::zero(
        T* dst, size_t count)
    {
        auto node = _graph.emplace_back(nstd::in_place_type_t<cudaNode::Memset>{},
                                        [dst, count](cudaGraph_t& graph, cudaGraphNode_t& node) {
                                            ...
                                            p.width = count;
                                            p.height = 1;
                                            TF_CHECK_CUDA(cudaGraphAddMemsetNode(&node, graph, nullptr, 0, &p),
    ...
    // Function: fill
    template <typename T>
    std::enable_if_t<is_pod_v<T> && (sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4), cudaTask> cudaFlow::fill(
        T* dst, T value, size_t count)
    {
        auto node = _graph.emplace_back(nstd::in_place_type_t<cudaNode::Memset>{},
    ...
    cudaTask cudaFlow::copy(T* tgt, const T* src, size_t num)
    {
        ...
        auto node = _graph.emplace_back(
            nstd::in_place_type_t<cudaNode::Copy>{}, [tgt, src, num](cudaGraph_t& graph, cudaGraphNode_t& node) {
                cudaMemcpy3DParms p;
                p.srcArray = nullptr;
                ...
                TF_CHECK_CUDA(cudaGraphAddMemcpyNode(&node, graph, nullptr, 0, &p),
    ...
    inline cudaTask cudaFlow::memset(void* dst, int ch, size_t count)
    {
        ...
        auto node = _graph.emplace_back(nstd::in_place_type_t<cudaNode::Memset>{},
                                        [dst, ch, count](cudaGraph_t& graph, cudaGraphNode_t& node) {
                                            ...
                                            // p.width = (count & 1) == 0 ? ((count & 3) == 0 ? count >> 2 : count >> 1)
                                            // : count;
