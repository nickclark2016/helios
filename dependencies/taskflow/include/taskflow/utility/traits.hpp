...
#define TF_HAS_MEMBER(class_, member_name) has_member_##member_name<class_>::value
    ...
    struct stateful_index
    {
        ...
        static_assert(std::is_integral<TB>::value, "decayed beg index must be an integral type");

        static_assert(std::is_integral<TE>::value, "decayed end index must be an integral type");

        static_assert(std::is_integral<TS>::value, "decayed step must be an integral type");

        static_assert(std::is_same<TB, TE>::value && std::is_same<TE, TS>::value,
