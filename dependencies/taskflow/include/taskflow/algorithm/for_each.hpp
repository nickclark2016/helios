...
namespace helios
{
    ...
    {
        return for_each_guided(std::forward<B>(beg), std::forward<E>(end), std::forward<C>(c), 1);
    }
    ...
    {
        return for_each_index_guided(std::forward<B>(beg), std::forward<E>(end), std::forward<S>(inc),
                                     std::forward<C>(c), 1);
    }
    ...
    Task FlowBuilder::for_each_guided(B&& beg, E&& end, C&& c, H&& chunk_size)
    {
        ...
        Task task = emplace([b = std::forward<B>(beg), e = std::forward<E>(end), c = std::forward<C>(c),
    ...
    Task FlowBuilder::for_each_index_guided(B&& beg, E&& end, S&& inc, C&& c, H&& chunk_size)
    {

        using I = stateful_index_t<B, E, S>;
        using namespace std::string_literals;

        Task task = emplace([b = std::forward<B>(beg), e = std::forward<E>(end), a = std::forward<S>(inc),
                             c = std::forward<C>(c), h = std::forward<H>(chunk_size)](Subflow& sf) mutable {
    ...
    // Function: for_each_dynamic
    template <typename B, typename E, typename C, typename H>
    Task FlowBuilder::for_each_dynamic(B&& beg, E&& end, C&& c, H&& chunk_size)
    {
    ...
    template <typename B, typename E, typename S, typename C, typename H>
    Task FlowBuilder::for_each_index_dynamic(B&& beg, E&& end, S&& inc, C&& c, H&& chunk_size)
    {
    ...
    template <typename B, typename E, typename C, typename H>
    Task FlowBuilder::for_each_static(B&& beg, E&& end, C&& c, H&& chunk_size)
    {
    ...
    // static scheduling with chunk size
    template <typename B, typename E, typename S, typename C, typename H>
    Task FlowBuilder::for_each_index_static(B&& beg, E&& end, S&& inc, C&& c, H&& chunk_size)
    {
