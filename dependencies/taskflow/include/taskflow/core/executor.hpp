...
namespace helios
{
    ...
    */
    ...
    public:
        ...
        */
        explicit Executor(size_t N = std::thread::hardware_concurrency(), size_t M = cuda_num_devices());
#else
        /**
        ...
        */
        ...
        @param taskflow a helios::Taskflow object
        ...
        */
        ...
        @param taskflow a helios::Taskflow
        @param N number of runs
        @param callable a callable object to be invoked after this run
        ...
        */
        ...
        @tparam Observer observer type derived from helios::ObserverInterface
    ...
    inline Executor::Executor(size_t N, size_t M)
        : _VICTIM_BEG{0}, _VICTIM_END{N + M - 1}, _MAX_STEALS{(N + M + 1) << 1}, _MAX_YIELDS{100}, _workers{N + M},
          _cuda_devices{cuda_num_devices()}, _notifier{Notifier(N), Notifier(M)}
    {
        ...
        for (size_t i = 0; i < _cuda_devices.size(); ++i)
        {
            ...
            {
                TF_CHECK_CUDA(cudaStreamCreate(&(_cuda_devices[i].streams[m])),
    ...
    inline Executor::Executor(size_t N)
        : _VICTIM_BEG{0}, _VICTIM_END{N - 1}, _MAX_STEALS{(N + 1) << 1}, _MAX_YIELDS{100}, _workers{N}, _notifier{
                                                                                                            Notifier(N)}
    {
    ...
    inline void Executor::_invoke_dynamic_work_internal(Worker& w, Node* p, Graph& g, bool detach)
    {
        ...
        else
        {
            ...
            exploit:
                ...
                explore:
                    t = (w.id == w.vtm) ? _wsq[w.domain].steal() : _workers[w.vtm].wsq[w.domain].steal();
                    if (t)
                    {
                        goto exploit;
                    }
    ...
    inline void Executor::_invoke_cudaflow_work_internal(Worker& w, Node* node)
    {
        ...
        auto s = cf._stream ? *(cf._stream) : _cuda_devices[d].streams[w.id - _id_offset[w.domain]];

        h.graph._make_native_graph();

        cudaGraphExec_t exec;

        TF_CHECK_CUDA(cudaGraphInstantiate(&exec, h.graph._native_handle, nullptr, nullptr, 0),
                      "failed to create an executable cudaGraph");

        while (!cf._predicate())
        {
            TF_CHECK_CUDA(cudaGraphLaunch(exec, s), "failed to launch cudaGraph on stream ", s);

            TF_CHECK_CUDA(cudaStreamSynchronize(s), "failed to synchronize stream ", s);
        }

        TF_CHECK_CUDA(cudaGraphExecDestroy(exec), "failed to destroy an executable cudaGraph");
