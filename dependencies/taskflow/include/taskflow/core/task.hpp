...
namespace helios
{
    ...
    */
    template <typename C>
    constexpr bool is_static_task_v = is_invocable_r_v<void, C> && !is_invocable_r_v<int, C>;
...
namespace std
{
    ...
    /**
    @class hash<helios::Task>

    @brief hash specialization for std::hash<helios::Task>

    */
    template <>
    struct hash<helios::Task>
    {
        auto operator()(const helios::Task& task) const noexcept
        {
