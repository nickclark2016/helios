...
namespace helios
{
    ...
    The helios::ExecutorObserver class let users define methods to monitor the behaviors
    of an executor.
        ...
        for (size_t w = first; w < _timeline.segments.size(); w++)
        {
            ...
            for (size_t i = 0; i < _timeline.segments[w].size(); i++)
            {
                ...
                os << "\"ph\":\"X\","
                   << "\"pid\":1,"
                   << "\"tid\":" << w << ',' << "\"ts\":"
                   << std::chrono::duration_cast<std::chrono::microseconds>(_timeline.segments[w][i].beg -
                                                                            _timeline.origin)
                          .count()
                   << ',' << "\"dur\":"
