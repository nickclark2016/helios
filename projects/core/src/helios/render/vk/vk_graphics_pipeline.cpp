...
namespace helios
{
    ...
    IGraphicsPipeline* GraphicsPipelineBuilder::build() const
    {
        ...
        {
            colorBlendAttachmentStates.push_back({static_cast<VkBool32>(attachment.blendEnable ? VK_TRUE : VK_FALSE),
        ...
        vertexInput.vertexBindingDescriptionCount = static_cast<u32>(vertexDescs.size());
        ...
        u32 sampleMask = _impl->multisample.sampleMask.has_value() ? _impl->multisample.sampleMask.value() : 0;
        ...
        colorBlend.attachmentCount = static_cast<u32>(colorBlendAttachmentStates.size());
        memcpy(colorBlend.blendConstants, _impl->colors.blendConstants, 4 * sizeof(f32));
        ...
        dynamic.dynamicStateCount = static_cast<u32>(_impl->dynamic.states.size());
        ...
        if (!destroyed)
        {
            if (!device->destroyed)
            {
                device->graphicsPipelines.erase(
                    std::find(device->graphicsPipelines.begin(), device->graphicsPipelines.end(), this));
            }
