...
namespace helios
{
	...
	static IPhysicalDevice::QueueProperties find_best_transfer(const vector<IPhysicalDevice::QueueProperties>& props)
	{
		...
		return { ~0UL, false, false, false, false, false, 0 };
	}
	
	static IPhysicalDevice::QueueProperties find_best_compute(const vector<IPhysicalDevice::QueueProperties>& props)
	{
		// first search for dedicated
		for (const auto& fam : props)
		{
	...
	static IPhysicalDevice::QueueProperties find_best_graphics(const vector<IPhysicalDevice::QueueProperties>& props)
	{
		// first search for dedicated
		for (const auto& fam : props)
		{
	...
	std::function<void(EMessageSeverity, const std::string)> ContextBuilder::DefaultCallback = [](EMessageSeverity severity, const std::string& message)
	{
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
		...
		std::function<void(EMessageSeverity, const char*)>& func = *static_cast<std::function<void(EMessageSeverity, const char*)>*>(pUserData);
	...
	struct ContextBuilder::ContextBuilderImpl
	{
		...
		uint32_t engineVersion = VK_MAKE_VERSION(HELIOS_VERSION_MAJOR, HELIOS_VERSION_MINOR, HELIOS_VERSION_PATCH);
		vector<std::string> extensions;
		vector<std::string> layers;
		std::function<void(EMessageSeverity, const char*)> debugCallback;
	};

	static std::unique_ptr<IPhysicalDevice> DefaultPhysicalDevice(nullptr);
	...
	struct DeviceBuilder::DeviceBuilderImpl
	{
		...
		uint32_t computeCount = ~0UL;
		uint32_t transferCount = ~0UL;
		uint32_t graphicsCount = ~0UL;
	};
	...
	struct VulkanPhysicalDevice final : IPhysicalDevice
	{
		...
		VulkanPhysicalDevice& operator=(VulkanPhysicalDevice&&) noexcept = delete;
	...
	struct VulkanSwapchain final : ISwapchain
	{
		...
		[[nodiscard]] uint32_t acquireNextImage(const uint64_t wait, const ISemaphore* signal, const IFence* fence) override;
	...
	struct VulkanCommandPool final : ICommandPool
	{
		...
		[[nodiscard]] vector<ICommandBuffer*> allocate(const uint32_t count,
	...
	struct VulkanCommandBuffer final : ICommandBuffer
	{
		...
		void beginRenderPass(const RenderPassRecordInfo& info, const bool isInline) override;
		void endRenderPass() override;
		void bind(const IGraphicsPipeline* pipeline) override;
		void draw(const uint32_t vertices, const uint32_t instances, const uint32_t baseVertex,
	...
	std::string stringify(const EPhysicalDeviceType& pType)
	{
		switch (pType)
		{
		case EPhysicalDeviceType::OTHER: return "OTHER";
		case EPhysicalDeviceType::INTEGRATED: return "INTEGRATED";
		case EPhysicalDeviceType::DISCRETE: return "DISCRETE";
		case EPhysicalDeviceType::VIRTUAL: return "VIRTUAL";
		case EPhysicalDeviceType::CPU: return "CPU";
		default: ;
		}
		return "UNKNOWN";
	}
	...
	ContextBuilder& ContextBuilder::applicationVersion(const uint32_t major, const uint32_t minor,
	...
	ContextBuilder& ContextBuilder::engineVersion(const uint32_t major, const uint32_t minor, const uint32_t revision)
	{
	...
	{
		{
			...
			for (uint32_t i = 0; i < extensionCount; i++)
			{
				...
				char* copy = reinterpret_cast<char*>(calloc(strlen(extension) + 1, 1));
				if (copy)
					strcpy(copy, extension);
				_impl->extensions.emplace_back(copy);
			}
		...
		info.enabledExtensionCount = static_cast<uint32_t>(_impl->extensions.size());
		...
		if (ctx && _impl->debugCallback)
		{
			...
			{
				fprintf(stderr, "Failed to load function vkCreateDebugUtilsMessengerEXT.  Extension not present.");
				delete ctx;
				ctx = nullptr;
			}
		}
	...
	DeviceBuilder& DeviceBuilder::features(const IPhysicalDevice::Features& features)
	{
	...
	IDevice* DeviceBuilder::build() const
	{
		...
		if (VulkanPhysicalDevice* vkDevPtr = dynamic_cast<VulkanPhysicalDevice*>(devPtr))
		{
			static bool init = gladLoaderLoadVulkan(vkDevPtr->context->instance, vkDevPtr->device, VK_NULL_HANDLE) > 0;
			if (!init)
			{
				exit(-1);
			}
		...
		{
			if (VulkanPhysicalDevice* vkDevPtr = cast<VulkanPhysicalDevice*>(devPtr))
			{
		...
		if (_impl->queues.empty())
		{
			...
			if (_impl->transferCount == ~0UL)
			{
				_impl->transferCount = 1;
			}

			if (_impl->computeCount == ~0UL)
			{
				_impl->computeCount = 1;
			}

			if (_impl->graphicsCount == ~0UL)
			{
			...
			{
				_impl->queues.push_back({ transfer.index, false, vector<float>(_impl->transferCount, 1.0f / static_cast<float>(_impl->transferCount)) });
			}

			if (_impl->computeCount > 0)
			{
				_impl->queues.push_back({ compute.index, false, vector<float>(_impl->computeCount, 1.0f / static_cast<float>(_impl->computeCount)) });
			}

			if (_impl->graphicsCount > 0)
			{
				_impl->queues.push_back({ graphics.index, false, vector<float>(_impl->graphicsCount, 1.0f / static_cast<float>(_impl->graphicsCount)) });
			}
		}

		std::sort(_impl->queues.begin(), _impl->queues.end(), [](QueueFamilyInfo& left, QueueFamilyInfo& right) { return left.family < right.family; });
		...
		for (const auto& info : _impl->queues)
		{
			...
			queue.flags |= info.protect ? VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT : 0;
		...
		info.queueCreateInfoCount = static_cast<uint32_t>(deviceQueueInfos.size());

		if (VulkanPhysicalDevice* vkDevPtr = dynamic_cast<VulkanPhysicalDevice*>(devPtr))
		{
			if (vkCreateDevice(vkDevPtr->device, &info, nullptr, &device->device) != VK_SUCCESS)
			{
				device = nullptr;
			}

			static bool init = gladLoaderLoadVulkan(vkDevPtr->context->instance, vkDevPtr->device, device->device) > 0;
			if (!init)
			{
				exit(-1);
			}
			...
			if (!_impl->queues.empty())
			{
				for (const auto& queueInfo : _impl->queues)
				{
					...
					for (const auto prop : queues)
					{
						if (prop.index == idx)
						{
							for (uint32_t i = 0; i < static_cast<uint32_t>(queueInfo.priorities.size()); i++)
							{
								VulkanQueue* queue = new VulkanQueue;
								queue->family = prop;
								queue->queueIndex = i;
								queue->queuePriority = queueInfo.priorities[i];
								vkGetDeviceQueue(device->device, queue->family.index, queue->queueIndex, &queue->queue);
			...
			else
			{
				...
				for (const auto prop : queues)
				{
					for (uint32_t i = 0; i < prop.count; i++)
					{
						...
						queue->family = prop;
						queue->queueIndex = i;
						queue->queuePriority = 1.0f / static_cast<float>(prop.count);
						vkGetDeviceQueue(device->device, queue->family.index, queue->queueIndex, &queue->queue);
						device->deviceQueues.push_back(queue);
					}
				}
	...
	ISwapchain* SwapchainBuilder::build() const
	{
		...
		for (const auto image : images)
		{
			...
			const auto view = ImageViewBuilder().type(EImageViewType::TYPE_2D)
				.image(img)
				.aspect(ASPECT_COLOR)
				.format(_impl->format)
				.build();
	...
	IImageView* ImageViewBuilder::build() const
	{
		...
		info.components = {
			static_cast<VkComponentSwizzle>(_impl->red),
		...
		info.subresourceRange = {
			static_cast<VkImageAspectFlags>(_impl->aspect),
			_impl->baseMip,
			_impl->mipLevels,
			_impl->baseArrayLayer,
			_impl->arrayLayers
		};
	...
	IPipelineLayout* PipelineLayoutBuilder::build() const
	{
		...
		pipelineLayout->device = _impl->layouts.empty() ? cast<VulkanDevice*>(_impl->device) : cast<VulkanDescriptorSetLayout*>(_impl->layouts[0])->device;
	...
	GraphicsPipelineBuilder& GraphicsPipelineBuilder::deriveFrom(const IGraphicsPipeline* parent)
	{
		_impl->parent = const_cast<IGraphicsPipeline*>(parent);
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::vertex(const IShaderModule* vertex)
	{
		_impl->vertex = const_cast<IShaderModule*>(vertex);
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::fragment(const IShaderModule* fragment)
	{
		_impl->fragment = const_cast<IShaderModule*>(fragment);
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::input(const VertexInputState& bindings)
	{
		_impl->bindings = bindings;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::assembly(const InputAssemblyState& assembly)
	{
		_impl->input = assembly;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::tessellation(const TessellationState& tessellation)
	{
		_impl->tessellation = tessellation;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::viewports(const ViewportState& viewport)
	{
		_impl->viewports = viewport;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::rasterization(const RasterizationState& raster)
	{
		_impl->rasterization = raster;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::multisample(const MultisampleState& multi)
	{
		_impl->multisample = multi;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::depthStencil(const DepthStencilState& depth)
	{
		_impl->depth = depth;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::colorBlend(const ColorBlendState& color)
	{
		_impl->colors = color;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::dynamic(const DynamicState& dynamic)
	{
		_impl->dynamic = dynamic;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::layout(const IPipelineLayout* layout)
	{
		_impl->layout = const_cast<IPipelineLayout*>(layout);
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::subpass(const uint32_t subpass)
	{
		_impl->subpass = subpass;
		return *this;
	}

	GraphicsPipelineBuilder& GraphicsPipelineBuilder::renderpass(const IRenderPass* renderpass)
	{
	...
	IGraphicsPipeline* GraphicsPipelineBuilder::build() const
	{
		...
		vector<VkPipelineShaderStageCreateInfo> stages;
		{
			if (_impl->vertex)
			{
				...
				stage.module = cast<VulkanShaderModule*>(_impl->vertex)->shaderModule;
			...
			if (_impl->fragment)
			{
				...
				stage.module = cast<VulkanShaderModule*>(_impl->fragment)->shaderModule;
				stage.pName = shader_entry.data();
		...
		for (const auto& attribBinding : _impl->bindings.bindings)
		{
			...
			desc.inputRate = static_cast<VkVertexInputRate>(attribBinding.inputRate);
		...
		{
			viewports.push_back({ viewport.x, viewport.y, viewport.width, viewport.height, viewport.minDepth, viewport.maxDepth });
		}
		...
		for (const auto& attachment : _impl->colors.attachments)
		{
			colorBlendAttachmentStates.push_back({
				static_cast<VkBool32>(attachment.blendEnable ? VK_TRUE : VK_FALSE),
		...
		vertexInput.vertexBindingDescriptionCount = static_cast<uint32_t>(vertexDescs.size());
		...
		assembly.primitiveRestartEnable = _impl->input.primitiveRestartEnable ? VK_TRUE : VK_FALSE;
		assembly.topology = static_cast<VkPrimitiveTopology>(_impl->input.topology);

		VkPipelineTessellationStateCreateInfo tessellation = {};
		tessellation.sType = VK_STRUCTURE_TYPE_PIPELINE_TESSELLATION_STATE_CREATE_INFO;
		tessellation.patchControlPoints = _impl->tessellation.patchControlPoints;
		...
		rasterization.depthBiasConstantFactor = _impl->rasterization.depthBiasConstantFactor;
		rasterization.depthBiasEnable = _impl->rasterization.depthBiasEnable ? VK_TRUE : VK_FALSE;
		rasterization.depthBiasSlopeFactor = _impl->rasterization.depthBiasSlopeFactor;
		rasterization.depthClampEnable = _impl->rasterization.depthClampEnable ? VK_TRUE : VK_FALSE;
		rasterization.frontFace = static_cast<VkFrontFace>(_impl->rasterization.frontFace);
		rasterization.lineWidth = _impl->rasterization.lineWidth;
		rasterization.polygonMode = static_cast<VkPolygonMode>(_impl->rasterization.mode);
		rasterization.rasterizerDiscardEnable = _impl->rasterization.rasterizerDiscardEnable ? VK_TRUE : VK_FALSE;

		uint32_t sampleMask = _impl->multisample.sampleMask.has_value() ? _impl->multisample.sampleMask.value() : 0;
		VkPipelineMultisampleStateCreateInfo multisample = {};
		multisample.sType = VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO;
		multisample.alphaToCoverageEnable = _impl->multisample.alphaToCoverage ? VK_TRUE : VK_FALSE;
		multisample.alphaToOneEnable = _impl->multisample.alphaToOne ? VK_TRUE : VK_FALSE;
		multisample.minSampleShading = _impl->multisample.minSampleShading;
		multisample.pSampleMask = _impl->multisample.sampleMask.has_value() ? &sampleMask : nullptr;
		multisample.rasterizationSamples = static_cast<VkSampleCountFlagBits>(_impl->multisample.samples);
		multisample.sampleShadingEnable = _impl->multisample.sampleShadingEnable ? VK_TRUE : VK_FALSE;

		VkPipelineDepthStencilStateCreateInfo depth = {};
		depth.sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
		depth.back = {
			static_cast<VkStencilOp>(_impl->depth.back.failOp),
		...
		depth.depthBoundsTestEnable = _impl->depth.depthBoundsTestEnable ? VK_TRUE : VK_FALSE;
		depth.depthCompareOp = static_cast<VkCompareOp>(_impl->depth.depthCompareOp);
		depth.depthTestEnable = _impl->depth.depthTestEnable ? VK_TRUE : VK_FALSE;
		depth.depthWriteEnable = _impl->depth.depthWriteEnable ? VK_TRUE : VK_FALSE;
		depth.front = {
			static_cast<VkStencilOp>(_impl->depth.front.failOp),
		...
		memcpy(colorBlend.blendConstants, _impl->colors.blendConstants, 4 * sizeof(float));
		colorBlend.logicOp = static_cast<VkLogicOp>(_impl->colors.logicOp);
		colorBlend.logicOpEnable = _impl->colors.logicEnable ? VK_TRUE : VK_FALSE;
		colorBlend.pAttachments = colorBlendAttachmentStates.data();

		VkPipelineDynamicStateCreateInfo dynamic = {};
		dynamic.sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO;
		dynamic.dynamicStateCount = static_cast<uint32_t>(_impl->dynamic.states.size());
		dynamic.pDynamicStates = dynamicStates.data();
		
		VkGraphicsPipelineCreateInfo info = {};
		info.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
		info.flags |= _impl->allowDerivatives ? VK_PIPELINE_CREATE_ALLOW_DERIVATIVES_BIT : 0;
		...
		info.basePipelineHandle = _impl->parent ? cast<VulkanGraphicsPipeline*>(_impl->parent)->pipeline : nullptr;
		info.basePipelineIndex = 0;
		info.renderPass = pipeline->renderpass->renderpass;

		vkCreateGraphicsPipelines(pipeline->device->device, nullptr, 1, &info, nullptr, &pipeline->pipeline);
	...
	IRenderPass* RenderPassBuilder::build() const
	{
		...
		for (const auto& sub : _impl->subpasses)
		{
			...
			for (const auto& ref : sub.inputAttachments)
			{
				inputs.push_back({
						ref.attachment,
						static_cast<VkImageLayout>(ref.layout)
					});
			}
			...
			{
				depth = {
					sub.depthAttachment.value().attachment,
			...
			subpasses.push_back({
					0,
					static_cast<VkPipelineBindPoint>(sub.bind),
	...
	VulkanPhysicalDevice::~VulkanPhysicalDevice()
	{
		if (!destroyed)
		{
			...
			{
				this->context->devices.erase(std::find(this->context->devices.begin(), this->context->devices.end(), this));
			}
	...
	{
		VkBool32 present;
		vkGetPhysicalDeviceSurfaceSupportKHR(cast<const VulkanPhysicalDevice*>(device)->device, family.index, cast<const VulkanSurface*>(surface)->surface, &present);
	...
	void VulkanQueue::submit(const vector<SubmitInfo>& submitInfo, const IFence* fence) const
	{
		...
		vkQueueSubmit(queue, static_cast<uint32_t>(infos.size()), infos.data(), cast<const VulkanFence*>(fence)->fence);
	}
	...
	ISurface::SwapchainSupport VulkanSurface::swapchainSupport(const IPhysicalDevice* device) const
	{
		...
		{
			formats.push_back({
				static_cast<EFormat>(format.format),
	...
	uint32_t VulkanSwapchain::acquireNextImage(const uint64_t wait, const ISemaphore* signal,
		const IFence* fence)
	{
	...
	VulkanPipelineLayout::~VulkanPipelineLayout()
	{
		if (!destroyed)
		{
			...
			if (!device->destroyed)
			{
				device->pipelineLayouts.erase(std::find(device->pipelineLayouts.begin(), device->pipelineLayouts.end(), this));
			}
	...
	VulkanGraphicsPipeline::~VulkanGraphicsPipeline()
	{
		if (!destroyed)
		{
			if (!device->destroyed)
			{
				device->graphicsPipelines.erase(std::find(device->graphicsPipelines.begin(), device->graphicsPipelines.end(), this));
			}
	...
	VulkanCommandPool::~VulkanCommandPool()
	{
		if (!destroyed)
		{
			...
			if (!device->destroyed)
			{
				device->commandBufferPools.erase(std::find(device->commandBufferPools.begin(), device->commandBufferPools.end(), this));
			}
	...
	ICommandBuffer* VulkanCommandPool::allocate(const ECommandBufferLevel level)
	{
		...
		info.level = level == ECommandBufferLevel::PRIMARY ? VK_COMMAND_BUFFER_LEVEL_PRIMARY : VK_COMMAND_BUFFER_LEVEL_SECONDARY;
	...
	vector<ICommandBuffer*> VulkanCommandPool::allocate(const uint32_t count,
	...
	VulkanCommandBuffer::~VulkanCommandBuffer()
	{
		if (!destroyed)
		{
			...
			if (!pool->destroyed)
			{
				pool->buffers.erase(std::find(pool->buffers.begin(), pool->buffers.end(), this));
			}
	...
	void VulkanCommandBuffer::beginRenderPass(const RenderPassRecordInfo& info, const bool isInline)
	{
		...
		beginInfo.renderPass = cast<VulkanRenderPass*>(info.renderpass)->renderpass;
		beginInfo.clearValueCount = static_cast<uint32_t>(values.size());
		beginInfo.pClearValues = values.data();

		vkCmdBeginRenderPass(buffer, &beginInfo, isInline ? VK_SUBPASS_CONTENTS_INLINE : VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS);
	}
	...
	void VulkanCommandBuffer::draw(const uint32_t vertices, const uint32_t instances, const uint32_t baseVertex,
	...
	{
		vkCmdBindPipeline(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, cast<const VulkanGraphicsPipeline*>(pipeline)->pipeline);
	}
