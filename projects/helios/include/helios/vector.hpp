...
namespace helios
{
	...
	template <typename Type>
	inline vector<Type>::vector(const size_t sz, const Type& value)
		: vector()
	{
		if (sz > 0)
		{
	...
	template <typename Type>
	vector<Type>::vector(std::initializer_list<Type> ilist)
		: vector()
	{
	...
	template <typename Type>
	template <typename InputIterator>
	vector<Type>::vector(InputIterator begin, InputIterator end)
		: vector()
	{
	...
	inline void vector<Type>::reserve(const size_t capacity)
	{
		...
		{
			Type* data = reinterpret_cast<Type*>(realloc(_data, sizeof(Type) * capacity));
			_data = data;
			_capacity = capacity;
		}
	}
