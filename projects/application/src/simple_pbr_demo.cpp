...
void simple_pbr::run()
{
    ...
    const auto window = WindowBuilder().title("Helios Window").width(1024).height(1024).build();
    ...
    void* pixels =
        stbi_load("res/models/barramundi/BarramundiFish_baseColor.png", &aWidth, &aHeight, &aChannels, STBI_rgb_alpha);
    ...
    pixels = stbi_load("res/models/barramundi/BarramundiFish_occlusionRoughnessMetallic.png", &mWidth, &mHeight,
    ...
    pixels =
        stbi_load("res/models/barramundi/BarramundiFish_normal.png", &nWidth, &nHeight, &nChannels, STBI_rgb_alpha);
    ...
    for (auto i = 0U; i < swapchain->imagesCount(); i++)
    {
        ...
        commandBuffers[i]->beginRenderPass(
            {renderpass, framebuffers[i], 0, 0, window->width(), window->height(), clear}, true);
