...
void simple_pbr::run()
{
    ...
    auto renderStateSetLayout =
        DescriptorSetLayoutBuilder()
            ...
            .bindings({{0, EDescriptorType::UNIFORM_BUFFER, 1,
                       ...
                       {1, EDescriptorType::UNIFORM_BUFFER, 1,
    ...
    const auto pipelineLayout =
        PipelineLayoutBuilder()
            .device(device)
            .layouts({renderStateSetLayout, modelStateSetLayout})
            .build();
    ...
    transitionCmd->barrier(
        ...
        {{0, ACCESS_TRANSFER_WRITE_BIT, EImageLayout::TRANSFER_DST_OPTIMAL,
          EImageLayout::SHADER_READ_ONLY_OPTIMAL, ~(0U), ~(0U), albedo,
    ...
    auto rendererSets =
        descriptorPool->allocate({renderStateSetLayout, renderStateSetLayout});

    auto modelSets =
        descriptorPool->allocate({modelStateSetLayout, modelStateSetLayout});
    ...
    for (const auto& set : rendererSets)
    {
        ...
        write.descriptorInfos =
            vector<DescriptorBufferInfo>({{rendererBuffer, 0, 256}});
        set->write({write});
        ...
        write.type = EDescriptorType::UNIFORM_BUFFER;
        write.descriptorInfos =
            vector<DescriptorBufferInfo>({{materialBuffer, 0, 256}});
        set->write({write});
    }
    ...
    for (const auto& set : modelSets)
    {
        ...
        write.type = EDescriptorType::UNIFORM_BUFFER;
        write.descriptorInfos =
            vector<DescriptorBufferInfo>({{modelBuffer, 0, 64 * 256}});
        set->write({write});
    }
