...
int main()
{
	...
	const auto ctx = ContextBuilder().applicationVersion(0, 0, 1)
		.applicationName("Helios Sample")
		.validation()
		.build();

	const auto window = WindowBuilder().title("Helios Window")
		.width(1280)
		.height(720)
		.build();
	...
	const auto device = DeviceBuilder().validation()
	...
	const auto surface = SurfaceBuilder().device(device)
		.window(window)
		.build();
	...
	const auto swapchain = SwapchainBuilder().surface(surface)
		.images(2)
		.width(window->width())
	...
	const auto vertexModule = ShaderModuleBuilder().device(device)
		.source(vertexSource)
		.build();
	const auto fragmentModule = ShaderModuleBuilder().device(device)
		.source(fragmentSource)
		.build();

	const auto pipelineLayout = PipelineLayoutBuilder().device(device)
		.build();

	const auto renderpass = RenderPassBuilder().device(device)
		.attachments({
			{
				swapchain->format(),
				SAMPLE_COUNT_1,
				...
				EAttachmentLoadOp::DONT_CARE,
				EAttachmentStoreOp::DONT_CARE,
	...
	const auto pipeline = GraphicsPipelineBuilder().vertex(vertexModule)
	...
	{
		framebuffers.push_back(FramebufferBuilder().attachments({ view })
			.renderpass(renderpass)
			.width(window->width())
			.height(window->height())
			.layers(1)
	...
	const auto commandPool = CommandPoolBuilder().device(device)
		.queue(presentQueue)
		.reset()
		.build();
