...
static void uploadMesh(helios::vector<helios::IBuffer*>& out, helios::IBuffer** elements, helios::IDevice* device,
...
void textured_cube::run()
{
    ...
    auto setLayout = DescriptorSetLayoutBuilder()
                         .device(device)
                         .bindings({
                             {0, EDescriptorType::UNIFORM_BUFFER, 1, SHADER_STAGE_VERTEX_BIT},
    ...
    void* pixels = stbi_load("res/models/cube/Cube_BaseColor.png", &width, &height, &channels, STBI_rgb_alpha);
